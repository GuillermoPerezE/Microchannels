...
__all__ = ['Ackley1', 'Ackley4', 'Alpine1', 'Alpine2', 'Bohachevsky', 'Brent', 'Brown', 'CarromTable', 'ChungReynolds',
           ...
           'Exponential', 'F2', 'Giunta', 'Griewank', 'HappyCat', 'HGBat', 'HyperEllipsoid', 'InvertedCosineWave',
...
class HGBat(BasicProblem):
    ...
    def get_func_val(self, variables, alpha=1. / 4., *args):
        x1 = np.sum(variables)
        x2 = np.sum(np.square(variables))
        return np.power(np.square(np.square(x2) - np.square(x1)), alpha) + (0.5 * x2 + x1) / (
            self.variable_num) + 0.5
